...
export function createLocalGateway(options: CreateLocalGatewayOptions = {}): CreateLocalGatewayResult {
  ...
  const gateway = new InProcessGateway(router, {
    ...
    async resolveTurnModelSelection(input) {
      ...
      // Native sessions have no SDK configuration marker and continue below.
      ...
      if (input.modelSelection !== undefined) {
        ...
        if (input.modelSelection.mode === "model") {
          registry.assertOrganizationModelAllowed(projectKey, input.modelSelection.provider, input.modelSelection.model);
          registry.assertManagedModelAllowed(input.sessionKey, input.modelSelection.provider, input.modelSelection.model);
        }
...
class ProjectRuntimeRegistry {
  ...
  private async prepareSessionRuntime(context: GatewaySessionContext) {
    ...
    const sandboxHiddenTools = sandboxHiddenToolNames(sdkConfig?.sandbox, sessionTools.list());
    ...
    if (deferredSdkTools.length > 0) {
      ...
      if (canExposeDeferredSearch) {
        ...
        const lateSandboxHiddenTools = sandboxHiddenToolNames(sdkConfig?.sandbox, sessionTools.list());
    ...
    const filterSubagentToolRegistry = (
      ...
      const sandboxHiddenTools = sandboxHiddenToolNames(sdkConfig?.sandbox, registry.list());
      const hiddenTools = new Set([...organizationHiddenTools, ...managedHiddenTools, ...sandboxHiddenTools]);
...
function validateSdkSandbox(
  ...
  if (!value) return;
  if ((value.type !== undefined && value.type !== "tool_policy")
...
function sandboxHiddenToolNames(
  ...
  for (const tool of tools) {
    ...
    if (denyHostBridge || denyFilesystem || denyNetwork) hidden.add(tool.name);
  }
  return hidden;
}
