...
export type InvocationStorageConfig = {
  root: string;
...
export function resolveInvocationStorageConfig(
  env: Record<string, string | undefined> = process.env,
): InvocationStorageConfig | undefined {
  const root = env.PILOTDECK_STORAGE_ROOT?.trim();
  if (!root) return undefined;
  return {
    root: resolve(root),
    snapshotRoot: env.PILOTDECK_SNAPSHOT_ROOT?.trim() || resolve(root),
    database: {
      url: env.PILOTDECK_DATABASE_URL?.trim() || undefined,
      schema: env.PILOTDECK_DATABASE_SCHEMA?.trim() || undefined,
      table: env.PILOTDECK_DATABASE_TABLE?.trim() || undefined,
    },
    storageConfigVersion: env.PILOTDECK_STORAGE_CONFIG_VERSION?.trim() || "env",
    objectPrefix: env.PILOTDECK_OBJECT_PREFIX?.trim() || undefined,
  };
}
...
export class JsonlInvocationLogSink implements ModelInvocationLogSink {
  ...
  constructor(private readonly config: InvocationStorageConfig) {}
