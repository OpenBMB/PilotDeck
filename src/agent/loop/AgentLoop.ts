...
function createNativeModelInvoker(
  ...
  return {
    async prepare({ request, context }): Promise<PreparedModelInvocation> {
      const decision = context.modelOverride
        ? {
            ...
            isSubagent: !options.isMainAgent,
            orchestrating: false,
      ...
      const materialized = router.materializeRequest
    ...
    stream: ({ prepared, context }) => {
      ...
      if (!decision) throw new Error("Agent model invoker received an unprepared invocation.");
...
function createNativeToolInvoker(
  registry: { list(): PilotDeckToolDefinition[] },
  scheduler: import("../../tool/index.js").PilotDeckToolScheduler,
): AgentToolInvoker {
  return {
    list: () => registry.list(),
    executeAll: (calls, context) => scheduler.executeAll(calls, context),
  };
}
...
export class AgentLoop {
  ...
  private readonly modelPort: AgentModelInvoker;
  private readonly toolPort: AgentToolInvoker;
  ...
  constructor(
    ...
    this.modelPort = createNativeModelInvoker(dependencies.router, {
      isMainAgent: !config.isSubagent,
      projectPath: config.cwd,
      fallbackModels: config.fallbackModels,
      managedModelPolicy: config.managedModelPolicy,
    });
    this.toolPort = createNativeToolInvoker(
