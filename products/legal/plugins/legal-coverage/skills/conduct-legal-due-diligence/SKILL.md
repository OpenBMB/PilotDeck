...
6. After each worker batch returns, follow the injected `sourceFragmentCommand` to load only the bounded validated rows instead of reading a complete fragment or replaying raw source text. In the next response, express your legal judgment in the exact injected source-merge proposal format; do not edit canonical ledgers manually. When the proposal receipt is valid, execute the injected `sourceMergeApplyCommand` exactly. Do not launch a second extraction wave while completed fragments remain unmerged.
