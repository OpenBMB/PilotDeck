...
6. After each worker batch returns, follow the injected `sourceFragmentCommand` to load only the bounded validated rows instead of reading a complete fragment or replaying raw source text, then immediately merge supported facts into canonical state. Do not launch a second extraction wave while completed fragments remain unmerged.
