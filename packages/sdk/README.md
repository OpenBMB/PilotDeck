...
`client.runs.start()` 可接受可选的 `runId`，该 ID 会原样透传到 Gateway 并出现在返回 handle 上；SDK/Gateway 会拒绝空白、过长或含空格的 ID，但本阶段不承诺同 ID 幂等。`runs.start()` 的 `attachments`、`uploadedAttachments` 和 `trustedContext` 只属于这一次 run，不放在 client defaults 或 `PilotDeckOptions` 中，因此不会自动注入下一轮。附件路径若使用，必须是 Gateway 主机路径；上传 artifact 的生命周期仍由 Gateway 管理。`trustedContext` 要求 `source`、`purpose` 和 `scope: "turn"`，且只有 Gateway 配置的可信宿主授权器通过后才会进入上下文；Gateway 记录授权 principal，不会把它提升为 system message。

远程调用方可使用 `client.uploads.create()`、`uploads.part()`、`uploads.complete()`、`uploads.get()` 和 `uploads.cancel()` 管理 Gateway-owned artifact，再把返回的 `uploadId`/`attachmentIds` 传给 `runs.start()`；上传记录不会向 SDK 暴露 Gateway 本地路径，单个 Remote API part 有界以避免无限制的 WebSocket 帧。
...
本版本不提供 host sandbox 或 Bubblewrap。`sandbox` 仅表示 Gateway-owned 的工具可见性策略，不创建 OS/container 隔离边界；传入 `type: "host"` 会返回 `unsupported_capability`。
