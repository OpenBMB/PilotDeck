...
| `OfficePreviewSections`, legacy `/settings/office`; binary-file Office preview controls and status | `workspace.office-preview`; host workspace/file API | `officePreviewStatus`, preview/preflight/download requests and refresh effects | Settings migrated. `CodeEditorBinaryFile` still owns DOCX/XLSX/PPTX renderer imports and Office status/blob requests, so file rendering is not yet statically removable. It requires a new editor-file-renderer contribution (separate from chat artifact renderers); generic text/image/PDF file viewing remains host-owned. |
