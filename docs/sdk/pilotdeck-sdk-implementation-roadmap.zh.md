...
| 技术差距 | 当前边界 | 后续前提 |
|---|---|---|
| 完整 host/OS-native `SandboxSettings`（不实施） | standalone SDK 只公开 `sandbox: { type: "tool_policy" }`，用于收紧 session-local model-visible ToolRegistry；它不创建进程、容器、文件系统或网络隔离边界。公共类型不包含 `type: "host"`，原始 wire 输入该类型时返回 `unsupported_capability`。 | 不进入当前 SDK roadmap；若未来另行立项，必须由宿主统一覆盖所有可执行面，并独立证明未配置时原生语义不变。 |
