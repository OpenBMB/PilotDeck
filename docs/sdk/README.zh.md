...
- PilotDeck 基线：`origin/main` 的提交 `cfc4d1779228f91fececc5d6705c14dab5b7ef2f`。
- 本 worktree 使用独立分支 `dev-sdk-0901-standalone`；不纳入其他 worktree 的未提交修改。
- 审计基线的根 package 标记为 `private: true`，没有 npm `exports`；实现交付新增 `packages/sdk` 的 `@pilotdeck/sdk` alpha package，独立使用 ESM `exports`。本 standalone 提交不包含 module/sidecar 协议。
...
- PilotDeck 已经有完整的 AgentLoop、canonical message/event、工具注册与调度、权限运行时、Session/Transcript、Hooks、Subagent、MCP、Skills/Plugins 和 Gateway 基础设施。
