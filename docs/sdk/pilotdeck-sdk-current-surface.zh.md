...
本文件描述 `origin/main@cfc4d177` 快照中的内部 TypeScript 导出和协议，不把它们定义为稳定 npm SDK。根 `package.json` 的 `private: true` 且没有 `exports`；源码路径和符号可能随内部重构变化。本 worktree 另有新增的 `packages/sdk` `@pilotdeck/sdk` alpha，它通过 Gateway adapter 提供独立 ESM `exports`，但不改变这一基线判断。
...
- `AgentRuntimeDependencies` 及原生 Router/Tool scheduler 依赖；
- `AgentLoopRunner` 仅作为 SDK E2E 的内部测试注入接口，不构成 module protocol。
...
Gateway 对 SDK `sandbox` 只实现 `tool_policy`：按 `filesystem`、`network`、`process` 收紧 session-local ToolRegistry。它是工具可见性策略，不是 OS/container sandbox；本提交不提供 named host profile 或 Bubblewrap。
...
本 standalone SDK 提交不包含 AgentLoop module protocol、sidecar CLI、sidecar factory 或 parity harness。SDK 通过 Gateway 或 embedded endpoint 调用原生 AgentLoop。
...
- 没有将 Gateway 和源码 exports 的兼容性等同于公共 API 稳定性。
