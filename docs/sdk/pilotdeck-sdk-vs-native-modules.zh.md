...
- embedded endpoint 复用相同 Gateway dispatcher，支持同进程调用；
...
- 开发新的 Gateway transport 或宿主 adapter；
...
需要跨进程调用 PilotDeck     -> 使用 Gateway WebSocket 或 MCP
