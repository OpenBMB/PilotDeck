...
本文以 @anthropic-ai/claude-agent-sdk@0.3.263 的 TypeScript 导出为 Claude 基线，以本 worktree 的 PilotDeck `origin/main@cfc4d177` 为原生语义基线，并记录本次实现的 `packages/sdk` Gateway-first alpha。
...
| sandbox | SDK `sandbox.tool_policy` → Gateway session ToolRegistry filter | 部分对应；仅提供 `filesystem`/`network`/`process` 的工具可见性收紧，不提供 host sandbox、Bubblewrap 或 OS/container 隔离。`type: "host"` 明确返回 `unsupported_capability`。 |
...
跨进程调用使用 Gateway/MCP；standalone SDK 提交不包含 AgentLoop module/sidecar protocol。
