...
|---|---|---|---|
| `permissionMode` | `PermissionMode`、Gateway mode、AgentSubmitOptions（`src/permission/index.ts`、`src/agent/protocol/input.ts`、`src/gateway/protocol/`） | 部分等价 | PilotDeck 由宿主保存权限状态和最终 allow/deny；SDK 不复制策略。 |
...
因此，“等价”只表示可完成同类任务；不表示类型、事件名称、持久化格式或跨进程兼容性相同。`src/` 下的导出属于本次提交快照的内部源码 API，Gateway 协议另行受其 wire schema 约束。
