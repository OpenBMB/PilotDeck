...
| `PilotDeckClient` 顶层 | `connect`, `describeServer`, `close`, `query`, `startup` | `types.ts:1350-1355`, `client.ts:2966-3035`; tests “client exposes Gateway-authoritative session, run and resource facades”, “client shares an in-flight handshake and run observation signals do not abort the Gateway run”, “startup returns a warmed query handle that reuses its handshake”, plus transport handshake/close rejection tests | covered; `connect`/`describeServer`/`close` also have direct transport evidence, while the facade test exercises the complete client composition |
