...
import { resolveToolUnit } from "../../../src/agent/policy/classify.js";
...
test("exact tool names win over the longest matching MCP prefix", () => {
  const limits = { mcp__: 9, mcp__faxin__: 3, mcp__faxin__search: 1 };
  assert.deepEqual(resolveToolUnit("mcp__faxin__search", limits), {
    unit: "mcp__faxin__search",
    limit: 1,
  });
  assert.deepEqual(resolveToolUnit("mcp__faxin__case", limits), {
    unit: "mcp__faxin__",
    limit: 3,
  });
});
...
test("main and subagent tool failures share one per-turn counter", () => {
  const guard = new FailureGuard(config({ toolFailureLimits: { mcp__faxin__: 2 } }));
  const main: AgentEvent = {
    type: "tool_result",
    sessionId: "s",
    turnId: "t",
    result: failure("mcp__faxin__law", "tool_execution_failed"),
  };
  const child: AgentEvent = {
    ...
    result: failure("mcp__faxin__case", "tool_unavailable"),
  };
  assert.equal(guard.observe(main), undefined);
  assert.equal(guard.observe(child)?.kind, "tool");

  const nextTurn = new FailureGuard(config({ toolFailureLimits: { mcp__faxin__: 2 } }));
