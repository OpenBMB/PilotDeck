...
test("gateway turns guidance into a canonical user message for the active run", async () => {
  ...
  let resolvedUploads: { projectKey: string; uploads: Array<{ uploadId: string; attachmentIds?: string[] }> } | undefined;
  const session = {
  ...
  const gateway = new InProcessGateway(router, {
    resolveUploadedAttachments: async (input) => {
      resolvedUploads = input;
      return [{ type: "text", name: "notes.txt", content: "uploaded guidance" }];
    },
  });
  ...
  assert.deepEqual(resolvedUploads, {
    projectKey: "/tmp/project",
    uploads: [{ uploadId: "upload-1", attachmentIds: ["attachment-1"] }],
  });
