  ...
  const gateway = new InProcessGateway(router, {
    resolveRunPolicy: () => ({
      failureGuard: {
        enabled: true,
        modelFailureLimit: 0,
        toolFailureLimits: { mcp__research__: 2 },
...
test("local Gateway scopes snapshot list, get, and restore to the owning project", async (t) => {
  ...
  const snapshotRoot = join(storageRoot, "snapshots");
  t.after(() => rm(projectRoot, { recursive: true, force: true }));
...
`, "utf8");
  ...
  const local = createLocalGateway({
    ...
    env: {
      PILOTDECK_STORAGE_ROOT: storageRoot,
      PILOTDECK_SNAPSHOT_ROOT: snapshotRoot,
    },
  });
  t.after(() => local.dispose());
