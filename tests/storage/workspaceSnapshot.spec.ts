...
test("workspace snapshot configuration is completely disabled without the storage root", () => {
  assert.equal(resolveWorkspaceSnapshotConfig({}), undefined);
  assert.deepEqual(resolveWorkspaceSnapshotConfig({
    PILOTDECK_STORAGE_ROOT: "/tmp/storage",
    PILOTDECK_SNAPSHOT_ROOT: "/tmp/snapshots",
  }), { root: "/tmp/storage", snapshotRoot: "/tmp/snapshots" });
});
