...
type ToolPermissionsSectionsProps = { title: string };

export default function ToolPermissionsSections({ title }: ToolPermissionsSectionsProps) {
  ...
  useEffect(() => {
    ...
    fetchPilotDeckPermissionSettings()
      ...
      .catch((error) => console.error("Failed to load permission settings from backend:", error));
  ...
  const handleAddAllowed = (value: string) => {
    ...
    setAllowedTools(next); setAskTools(nextAsk);
    persistPermissionSettings({ allowedTools: next, askTools: nextAsk, disallowedTools: nextDisallowed });
    setNewAllowed(""); showSettingsSuccess(t("permissions.feedback.allowedAdded", { tool }));
  };
  const handleRemoveAllowed = (value: string) => {
    const next = removeValue(allowedTools, value);
    setAllowedTools(next); persistPermissionSettings({ allowedTools: next });
  ...
  const handleAddApproval = (value: string) => {
    ...
    const nextDisallowed = removeValue(settings.disallowedTools, tool);
    setAskTools(next); setAllowedTools(nextAllowed);
    persistPermissionSettings({ askTools: next, allowedTools: nextAllowed, disallowedTools: nextDisallowed });
    setNewApproval(""); showSettingsSuccess(t("permissions.feedback.approvalAdded", { tool }));
  };
  const handleRemoveApproval = (value: string) => {
    const next = removeValue(askTools, value);
    setAskTools(next); persistPermissionSettings({ askTools: next });
      ...
      <section className="security-section" aria-labelledby="security-permissions-title">
        <div className="security-card security-permissions-card">
          <header className="security-card-header">
            <span className="security-card-header-icon" aria-hidden="true"><ShieldCheck size={19} /></span>
            <div><h2 id="security-permissions-title">{t("permissions.controlTitle")}</h2><p>{t("permissions.controlDescription")}</p></div>
          </header>
          <div className="security-permission-grid">
            <PermissionRulesSection mode="allowed" tools={allowedTools} newValue={newAllowed} onNewValueChange={setNewAllowed} onAdd={handleAddAllowed} onRemove={handleRemoveAllowed} quickTools={QUICK_ADD_TOOLS} />
            <PermissionRulesSection mode="approval" tools={askTools} newValue={newApproval} onNewValueChange={setNewApproval} onAdd={handleAddApproval} onRemove={handleRemoveApproval} quickTools={QUICK_BLOCK_TOOLS} />
          </div>
        </div>
      </section>
    </div>
